        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="max-w-6xl mx-auto mb-20 bg-slate-900 dark:bg-black rounded-[3rem] p-10 md:p-16 relative overflow-hidden border border-slate-800 shadow-2xl">
                 ...
                 <div className="relative z-10">
                    <div className="flex flex-col md:flex-row gap-12 items-center">
                        ...
                        <div className="w-full md:w-1/2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 md:p-8 mt-8 md:mt-0">
                            <h4 className="text-white font-bold text-lg md:text-xl mb-4 md:mb-6 flex items-center gap-2 md:gap-3">
                            ...
                            <div className="mt-8 md:mt-10 p-3 md:p-4 rounded-2xl bg-vibrant-orange/10 border border-vibrant-orange/20">
                                <p className="text-[10px] md:text-xs text-vibrant-orange font-bold text-center">
            ...
            <div className="max-w-6xl mx-auto mb-20 px-4">
                <div className="text-center mb-8 md:mb-12">
                    <h2 className="text-2xl md:text-3xl font-black text-navy-900 dark:text-white mb-2">Coin Usage Transparency</h2>
                    <p className="text-slate-500 dark:text-slate-400 font-bold uppercase text-[10px] md:text-xs tracking-widest text-center">Know exactly where your coins go.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    {/* Menu Card */}
                    <div className="bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border border-slate-200 dark:border-slate-800 shadow-xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 md:p-6 opacity-5"><History className="w-24 h-24 md:w-32 md:h-32"/></div>
                        <h4 className="text-lg md:text-xl font-black text-navy-900 dark:text-white mb-6 md:mb-8 flex items-center gap-2 md:gap-3">
                            <span className="w-1.5 md:w-2 h-6 md:h-8 bg-vibrant-orange rounded-full"></span>
                            Menu Card (Coin Cost)
                        </h4>
                        
                        <div className="space-y-3 md:space-y-5">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 md:p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 gap-3 sm:gap-0">
                                <div className="flex flex-col">
                                    <span className="text-navy-900 dark:text-white font-bold flex items-center gap-2 text-sm md:text-base">
                                        💬 Simple Chat
                                    </span>
                                    <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Query Handling</span>
                                </div>
                                <span className="font-black text-vibrant-orange bg-white dark:bg-slate-800 px-3 md:px-4 py-1.5 md:py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-xs md:text-sm whitespace-nowrap">1 Coin 🪙</span>
                            </div>
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 md:p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 gap-3 sm:gap-0">
                                <div className="flex flex-col">
                                    <span className="text-navy-900 dark:text-white font-bold flex items-center gap-2 text-sm md:text-base">
                                    ...
                                    <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Invoice, Live Consulting</span>
                                </div>
                                <span className="font-black text-vibrant-orange bg-white dark:bg-slate-800 px-3 md:px-4 py-1.5 md:py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-xs md:text-sm whitespace-nowrap">3 Coins 🪙</span>
                            </div>
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 md:p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 gap-3 sm:gap-0">
                                <div className="flex flex-col">
                                    <span className="text-navy-900 dark:text-white font-bold flex items-center gap-2 text-sm md:text-base">
                                    ...
                                    <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Multiple Tools, Invoice, PDF, Live Data</span>
                                </div>
                                <span className="font-black text-vibrant-orange bg-white dark:bg-slate-800 px-3 md:px-4 py-1.5 md:py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-xs md:text-sm whitespace-nowrap">7 Coins 🪙</span>
                            </div>
                        </div>
                        <p className="text-[9px] md:text-[10px] text-slate-400 mt-4 md:mt-6 flex items-start gap-2 leading-relaxed font-bold uppercase tracking-widest">
                            <Sparkles className="w-3 h-3 md:w-4 md:h-4 text-vibrant-orange flex-shrink-0 mt-0.5" />
                            *Inventory Alarm + MONITORING + Calculation + Sending + Dashboard Analyze, & Multi Levels Complex Task's.
                        </p>
                    </div>

                    {/* Low Balance Alert */}
                    <div className="bg-red-50 dark:bg-red-900/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border-2 border-dashed border-red-500 shadow-xl flex flex-col items-center justify-center text-center relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:rotate-12 transition-transform"><AlertTriangle className="w-24 h-24 md:w-40 md:h-40 text-red-500"/></div>
                        <div className="shrink-0 mb-4 md:mb-6 relative">
                                <img src="https://i.ibb.co/k60hZRq6/image.png" alt="Genie Alert" className="w-24 md:w-32 h-auto drop-shadow-2xl animate-bounce-slow" />
                        </div>
                        <h4 className="text-xl md:text-2xl font-black text-red-600 dark:text-red-400 mb-2 md:mb-4 uppercase tracking-tighter">
                            Low Balance Alert! 🚨
                        </h4>
                        <p className="text-slate-700 dark:text-slate-300 text-xs md:text-sm font-bold leading-relaxed mb-4 md:mb-6 max-w-xs">
                            Your Genie Agent will automatically alert you when your balance drops below 50 coins.
                        </p>
                        <button className="px-6 md:px-8 py-2.5 md:py-3 bg-red-600 text-white font-black rounded-xl uppercase text-[10px] md:text-xs tracking-widest shadow-lg shadow-red-900/20 hover:bg-red-700 transition-all">
                            Set Alert Threshold
                        </button>
                    </div>
                </div>
