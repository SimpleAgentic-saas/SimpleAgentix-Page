...
const problems = [
  {
    key: "p1",
    fallback: "Is Your Staff Wasting Hours Doing The Same Repetitive Tasks Every Single Day?",
    icon: Repeat,
  ...
  {
    key: "p2",
    fallback: "Are You Facing Project Delays or Losing Money Because Of Simple Human Mistakes?",
    icon: AlertTriangle,
    color: "text-red-500",
  ...
  {
    key: "p3",
    fallback: "Are Your Profit Margins Shrinking Because You Have To Hire More People Just for Basic Work?",
    icon: TrendingDown,
    color: "text-rose-500",
  ...
  {
    key: "p4",
    fallback: "Are You Spending Half Your Day Managing WhatsApp Support, Excel Sheets, & Data Entry?",
    icon: Clock,
...
export const ProblemSection: React.FC<ProblemSectionProps> = ({ t }) => {
  const fullTitle = t?.title || "Are Manual Tasks Eating Your Business Profits?";
  const titleWords = fullTitle.split(' ');
  const highlightCount = titleWords.length > 3 ? 2 : 1;
  const mainTitle = titleWords.slice(0, -highlightCount).join(' ');
  const highlightText = titleWords.slice(-highlightCount).join(' ');
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-slate-900/90 to-[#0B1120]/90 dark:from-[#0B1120]/95 dark:to-black/95 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col group ring-1 ring-white/5">
                          ...
                          <div className="flex-1 relative p-8 z-10">
                              ...
                              <div className="absolute inset-0 flex items-center justify-center">
                                  ...
                                  <div className="absolute left-4 md:left-6 top-1/4 md:top-1/2 -translate-y-1/2 flex flex-row md:flex-col gap-4 md:gap-8 z-20">
                                      <div className="relative group/node">
                                          <div className="p-3 md:p-4 bg-slate-900/80 backdrop-blur-md border border-red-500/30 rounded-2xl shadow-[0_0_20px_rgba(239,68,68,0.1)] w-32 md:w-40 transform hover:scale-105 transition-all duration-300">
                                              <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                                                  <div className="p-1.5 rounded-lg bg-red-500/20 text-red-400"><AlertTriangle className="w-3 h-3 md:w-4 md:h-4" /></div>
                                                  <span className="text-[10px] md:text-xs font-bold text-red-100">Manual Errors</span>
                                              </div>
                                              <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                                  <div className="h-full bg-red-500 w-[80%] animate-pulse"></div>
                                              </div>
                                          </div>
                                          <div className="hidden md:block absolute top-1/2 -right-12 h-[1px] w-12 bg-gradient-to-r from-red-500/50 to-transparent"></div>
                                      </div>

                                      <div className="relative group/node">
                                          <div className="p-3 md:p-4 bg-slate-900/80 backdrop-blur-md border border-orange-500/30 rounded-2xl shadow-[0_0_20px_rgba(249,115,22,0.1)] w-32 md:w-40 transform hover:scale-105 transition-all duration-300 delay-150">
                                              <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                                                  <div className="p-1.5 rounded-lg bg-orange-500/20 text-orange-400"><Clock className="w-3 h-3 md:w-4 md:h-4" /></div>
                                                  <span className="text-[10px] md:text-xs font-bold text-orange-100">Delay Risk</span>
                                              </div>
                                              <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                                  <div className="h-full bg-orange-500 w-[60%] animate-pulse" style={{animationDelay: '0.5s'}}></div>
                                              </div>
                                          </div>
                                          <div className="hidden md:block absolute top-1/2 -right-12 h-[1px] w-12 bg-gradient-to-r from-orange-500/50 to-transparent"></div>
                                      </div>
                                  </div>

                                  {/* Central AI Core */}
                                  <div className="relative z-10 mt-16 md:mt-0">
                                      <div className="w-20 h-20 md:w-28 md:h-28 rounded-3xl bg-[#0B1120] border border-slate-700 shadow-2xl flex items-center justify-center relative group-hover:scale-110 transition-transform duration-500 z-20">
                                          <div className="absolute inset-0 bg-vibrant-green/20 blur-2xl rounded-full animate-pulse"></div>
                                          <Zap className="w-8 h-8 md:w-12 md:h-12 text-vibrant-green relative z-10 drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]" />
                                          
                                          {/* Orbiting Rings */}
                                          <div className="absolute -inset-2 md:-inset-4 border border-vibrant-green/20 rounded-[1.5rem] md:rounded-[2rem] rotate-45 animate-[spin_8s_linear_infinite]"></div>
                                          <div className="absolute -inset-2 md:-inset-4 border border-dashed border-vibrant-green/20 rounded-[1.5rem] md:rounded-[2rem] -rotate-12 animate-[spin_12s_linear_infinite_reverse]"></div>
                                      </div>
                                      
                                      {/* Output Line */}
                                      <div className="absolute top-full left-1/2 -translate-x-1/2 w-[2px] h-8 md:h-12 bg-gradient-to-b from-vibrant-green to-transparent"></div>
                                  </div>

                                  {/* Success Output (Bottom) */}
                                  <div className="absolute bottom-6 md:bottom-10 bg-[#0B1120] border border-green-500/30 rounded-2xl px-4 md:px-6 py-2 md:py-3 flex items-center gap-3 md:gap-4 shadow-[0_10px_40px_-10px_rgba(16,185,129,0.3)] animate-in slide-in-from-bottom-8 fade-in duration-700 delay-300 backdrop-blur-xl z-20">
                                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-green-500/10 flex items-center justify-center border border-green-500/20">
                                          <ShieldCheck className="w-4 h-4 md:w-5 md:h-5 text-vibrant-green" />
                                      </div>
                                      <div>
                                          <div className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider">Status</div>
                                          <span className="text-xs md:text-sm font-bold text-white">System Optimized</span>
                                      </div>
