...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode, onAuthSuccess }) => {
  ...
  // Stages: 'credentials' -> 'onboarding' (location/apps) -> 'verification' -> 'success'
  const [authStep, setAuthStep] = useState<'credentials' | 'onboarding' | 'verification' | 'success'>('credentials');
  ...
  const handleOnboardingSubmit = () => {
      ...
      setTimeout(() => {
          ...
          handleFinalSuccess({
              ...
              mobileNumber: connectionDetails.mobileNumber,
              address: connectionDetails.address,
              profilePic: connectionDetails.profilePic,
              isNewUser: true
          });
      }, 2000);
  };

  const handleFinalSuccess = async (profile: UserProfile) => {
      setAuthStep('success');
      ...
      <div className="relative w-full max-w-5xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="w-full md:w-7/12 p-8 md:p-12 flex flex-col justify-center bg-white dark:bg-slate-900 transition-colors relative overflow-y-auto max-h-[90vh]">
          ...
          {authStep === 'success' ? (
            ...
            <div className="flex flex-col items-center justify-center text-center space-y-6 py-12 animate-in zoom-in fade-in duration-300">
                <div className="w-24 h-24 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-full flex items-center justify-center">
                   <Mail className="w-12 h-12" />
                </div>
                <div>
                    <h3 className="text-2xl font-bold text-navy-900 dark:text-white mb-2">Verify Your Email</h3>
                    <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                        We have sent you a verification email to <span className="font-bold text-navy-900 dark:text-white">{verificationEmail}</span>. Please verify it and log in.
                    </p>
                </div>
                <button 
                  onClick={() => {
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                         ...
                         <div className="col-span-2">
                             ...
                             <input 
                                type="text"
                                value={connectionDetails.address}
                                onChange={(e) => setConnectionDetails({...connectionDetails, address: e.target.value})}
