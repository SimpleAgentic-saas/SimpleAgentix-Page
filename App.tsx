...
import AgenticFeaturesSection from './components/AgenticFeaturesSection';
...
const App: React.FC = () => {
  ...
  // Load user from local storage and sync with Firebase
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        if (!firebaseUser.emailVerified) {
          // If user is not verified, we don't let them in
          setUser(null);
        ...
        if (savedUser) {
          ...
          // If Firebase has a user but localStorage doesn't (e.g. after refresh)
  ...
  const handleLogin = (userProfile: UserProfile) => {
    ...
    // Redirect to Industry-Specific Solutions (gallery) if it's a new user (signup)
  ...
  if (user && viewMode === 'dashboard') {
    return (
      <Dashboard 
        ...
        initialNiche={pendingNicheSelection}
        initialFeature={pendingFeature}
        onHandoffComplete={() => {
          setPendingWorkflow(null);
          setPendingNicheSelection(null);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#020617] text-slate-900 dark:text-white font-sans selection:bg-vibrant-orange/30 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <button onClick={() => scrollToSection('faq')} className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-vibrant-orange transition-colors">{t.nav.faq}</button>
              
              {user && (
                <>
                  <button onClick={() => { setViewMode('landing'); setTimeout(() => scrollToSection('workflows'), 100); }} className="text-sm font-bold text-vibrant-orange hover:text-orange-600 transition-colors">Smart Agent</button>
                  <button onClick={() => setViewMode('dashboard')} className="text-sm font-bold text-blue-500 hover:text-blue-600 transition-colors">My Dashboard</button>
                  <button onClick={() => { setViewMode('dashboard'); /* Assuming profile is in dashboard */ }} className="text-sm font-bold text-emerald-500 hover:text-emerald-600 transition-colors">User Profile Dashboard</button>
                </>
              )}
        ...
        {mobileMenuOpen && (
           <div className="md:hidden bg-white dark:bg-[#020617] border-t border-slate-200 dark:border-slate-800 px-4 py-6 space-y-4 animate-in slide-in-from-top-4 shadow-2xl">
              ...
              <button onClick={() => scrollToSection('faq')} className="block w-full text-left text-sm font-medium text-slate-600 dark:text-slate-300 py-2">{t.nav.faq}</button>
              
              {user && (
                <>
                  <button onClick={() => { setViewMode('landing'); setMobileMenuOpen(false); setTimeout(() => scrollToSection('workflows'), 100); }} className="block w-full text-left text-sm font-bold text-vibrant-orange py-2">Smart Agent</button>
                  <button onClick={() => { setViewMode('dashboard'); setMobileMenuOpen(false); }} className="block w-full text-left text-sm font-bold text-blue-500 py-2">My Dashboard</button>
                  <button onClick={() => { setViewMode('dashboard'); setMobileMenuOpen(false); }} className="block w-full text-left text-sm font-bold text-emerald-500 py-2">User Profile Dashboard</button>
                </>
              )}
                 ...
                 <div className="flex gap-2">
                    {['English', 'Hindi', 'Bengali'].map(lang => (
                        <button 
                          ...
                          className={`px-3 py-1 text-xs font-bold rounded-full ${language === lang ? 'bg-vibrant-orange text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}
                        >
        ...
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          >
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-8"
            >
            ...
            <motion.h1 
              ...
              className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-navy-900 dark:text-white leading-[1.1] mb-8 tracking-tight max-w-5xl"
            >
               <span className="block mb-4">{t.hero.headline}</span>
               <span className="text-transparent bg-clip-text bg-gradient-to-r from-vibrant-orange to-red-500 block">
                 {t.hero.subhead}
               </span>
            </motion.h1>
            ...
            <motion.p 
              ...
              className="text-sm sm:text-lg text-slate-600 dark:text-slate-400 mb-10 max-w-3xl leading-relaxed px-4"
            >
               {t.hero.desc}
            </motion.p>
      ...
      <AgenticFeaturesSection language={language} />
      <div id="features"><ServiceFeaturesSection onLanguageChange={setLanguage} /></div>
      <UseCasesSection />
      <div id="gallery"><GallerySection language={language} onGeneratePlan={handleNicheHandoff}/></div>
      <div id="workflows">
        <AgenticWorkflowsSection 
          ...
          onLaunch={(workflowId) => {
            setPendingWorkflow(workflowId || 'custom');
            if (user) {
              // If already logged in, this won't be hit because of the early return, 
              // but good for consistency if we change the logic later.
            } else {
              openAuth('signup');
            }
          }}
      ...
      <div id="faq">
        <FAQSection language={language} faqData={(t as any).faq} />
      </div>
